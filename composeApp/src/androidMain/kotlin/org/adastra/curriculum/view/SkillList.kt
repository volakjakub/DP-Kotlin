...
@Composable
fun SkillList(biographyService: BiographyService, biography: BiographyResponse) {
    var isLoadingSkills by remember { mutableStateOf(true) }
    var error by remember { mutableStateOf<String?>(null) }
    val skillsState =
        produceState<List<SkillResponse>?>(initialValue = null) {
            try {
                value =
                    biographyService.getSkillsByBiography(
                        biography.id
                    )
            } catch (e: Exception) {
                error = e.message
